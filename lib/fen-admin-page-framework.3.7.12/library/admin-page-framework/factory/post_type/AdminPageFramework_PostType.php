...
abstract class fen_AdminPageFramework_PostType_Router extends fen_AdminPageFramework_Factory {
...
abstract class fen_AdminPageFramework_PostType_Model extends fen_AdminPageFramework_PostType_Router {
    public function __construct($oProp) {
        parent::__construct($oProp);
        ...
        if ($this->oProp->bIsAdmin) {
            ...
            if ($this->oProp->sCallerPath) {
                new fen_AdminPageFramework_PostType_Model__FlushRewriteRules($this);
            }
        }
    }
    ...
    public function _replyToRegisterPostType() {
        ...
        new fen_AdminPageFramework_PostType_Model__SubMenuOrder($this);
    }
...
abstract class fen_AdminPageFramework_PostType_View extends fen_AdminPageFramework_PostType_Model {
    public function __construct($oProp) {
        parent::__construct($oProp);
        if ($this->oProp->bIsAdmin) {
    ...
    public function _replyToPrintStyle() {
        ...
        if (!empty($this->oProp->sStyle)) {
            echo "<style type='text/css' id='fen-style-post-type'>" . $this->oProp->sStyle . "</style>";
        }
    }
...
abstract class fen_AdminPageFramework_PostType_Controller extends fen_AdminPageFramework_PostType_View {
...
abstract class fen_AdminPageFramework_PostType extends fen_AdminPageFramework_PostType_Controller {
    protected $_sStructureType = 'post_type';
    public function __construct($sPostType, $aArguments = array(), $sCallerPath = null, $sTextDomain = 'fen') {
        if (empty($sPostType)) {
            return;
        }
        ...
        $this->oProp->sPostType = fen_AdminPageFramework_WPUtility::sanitizeSlug($sPostType);
        $this->oProp->aPostTypeArgs = $aArguments;
    ...
    private function _getCallerScriptPath($sCallerPath) {
        ...
        $_sPageNow = fen_AdminPageFramework_Utility::getElement($GLOBALS, 'pagenow');
        if (in_array($_sPageNow, array('edit.php', 'post.php', 'post-new.php', 'plugins.php', 'tags.php', 'edit-tags.php',))) {
            return fen_AdminPageFramework_Utility::getCallerScriptPath(__FILE__);
        }
        return null;
    }
}
