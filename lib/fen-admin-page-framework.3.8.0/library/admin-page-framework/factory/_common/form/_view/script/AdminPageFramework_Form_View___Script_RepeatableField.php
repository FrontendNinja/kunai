...
class fen_AdminPageFramework_Form_View___Script_RepeatableField extends fen_AdminPageFramework_Form_View___Script_Base {
...
(function ( $ ) {
     ...
     */
    $.fn.updatefen_AdminPageFrameworkRepeatableFields = function( aSettings ) {
        ...
        if ( ! $.fn.afen_AdminPageFrameworkRepeatableFieldsOptions ) {
            $.fn.afen_AdminPageFrameworkRepeatableFieldsOptions = [];
        }
        if ( ! $.fn.afen_AdminPageFrameworkRepeatableFieldsOptions.hasOwnProperty( _sFieldsContainerID ) ) {     
            $.fn.afen_AdminPageFrameworkRepeatableFieldsOptions[ _sFieldsContainerID ] = $.extend({    
        ...
        var _aOptions = $.fn.afen_AdminPageFrameworkRepeatableFieldsOptions[ _sFieldsContainerID ];
...
// @todo For nested fields, the `find()` method should be avoided.
        $( nodeThis ).find( '.fen-repeatable-field-buttons' ).attr( 'data-max', _aOptions[ 'max' ] );
        $( nodeThis ).find( '.fen-repeatable-field-buttons' ).attr( 'data-min', _aOptions[ 'min' ] );
        $( nodeThis ).find( '.fen-repeatable-field-buttons' ).attr( 'data-fadein', _aOptions[ 'fadein' ] );
        $( nodeThis ).find( '.fen-repeatable-field-buttons' ).attr( 'data-fadeout', _aOptions[ 'fadeout' ] );
...
// @todo For nested fields, the `find()` method should be avoided.         
        ...
        $( nodeThis ).find( '.repeatable-field-add-button' ).click( function() {
            $( this ).addfen_AdminPageFrameworkRepeatableField();
...
// @todo For nested fields, the `find()` method should be avoided.        
        ...
        $( nodeThis ).find( '.repeatable-field-remove-button' ).click( function() {
            $( this ).removefen_AdminPageFrameworkRepeatableField();
            return false; // will not click after that
        });

        /* If the number of fields is less than the set minimum value, add fields. */
        var _sFieldID           = nodeThis.find( '.repeatable-field-add-button' ).first().closest( '.fen-field' ).attr( 'id' );
        var _nCurrentFieldCount = jQuery( '#' + _sFieldsContainerID ).find( '.fen-field' ).length;
        if ( _aOptions[ 'min' ] > 0 && _nCurrentFieldCount > 0 ) {
            if ( ( _aOptions[ 'min' ] - _nCurrentFieldCount ) > 0 ) {     
                $( '#' + _sFieldID ).addfen_AdminPageFrameworkRepeatableField( _sFieldID );  
            }
        }
     ...
     */
    $.fn.addfen_AdminPageFrameworkRepeatableField = function( sFieldContainerID ) {
        
        if ( 'undefined' === typeof sFieldContainerID ) {
            var sFieldContainerID = $( this ).closest( '.fen-field' ).attr( 'id' );
        }

        var nodeFieldContainer  = $( '#' + sFieldContainerID );
        var nodeNewField        = nodeFieldContainer.clone(); // clone without bind events.
        var nodeFieldsContainer = nodeFieldContainer.closest( '.fen-fields' );
        var _sFieldsContainerID = nodeFieldsContainer.attr( 'id' );

        // If the set maximum number of fields already exists, do not add.
        if ( ! $.fn.afen_AdminPageFrameworkRepeatableFieldsOptions.hasOwnProperty( _sFieldsContainerID ) ) {     
            var nodeButtonContainer = nodeFieldContainer.find( '.fen-repeatable-field-buttons' );
            $.fn.afen_AdminPageFrameworkRepeatableFieldsOptions[ _sFieldsContainerID ] = {    
        ...
        var _iFadein  = $.fn.afen_AdminPageFrameworkRepeatableFieldsOptions[ _sFieldsContainerID ][ 'fadein' ];
        var _iFadeout = $.fn.afen_AdminPageFrameworkRepeatableFieldsOptions[ _sFieldsContainerID ][ 'fadeout' ];

        // Show a warning message if the user tries to add more fields than the number of allowed fields.
        var sMaxNumberOfFields = $.fn.afen_AdminPageFrameworkRepeatableFieldsOptions[ _sFieldsContainerID ]['max'];
        if ( sMaxNumberOfFields != 0 && nodeFieldsContainer.find( '.fen-field' ).length >= sMaxNumberOfFields ) {
            var nodeLastRepeaterButtons = nodeFieldContainer.find( '.fen-repeatable-field-buttons' ).last();
         ...
         */
        nodeNewField.updatefen_AdminPageFrameworkRepeatableFields();
        ...
        // For nested fields,
/*         $( nodeNewField ).find( '.fen-field' ).each( function( iIterationIndex ) {    
...
// @todo find() may not be appropriate for nested fields.
        ...
        nodeFieldsContainer.children( '.fen-field' ).children( '.delimiter' ).show().last().hide();
        ...
        var _incrementFieldAttributes = function( oElement, oFieldsContainer ) {
            ...
            oElement.find( '.fen-field,.fen-fields,.fen-fieldset' ).incrementAttributes(
...
// console.log( oElement.find( 'fieldset.fen-fieldset' ).length );
// console.log( oElement.find( 'fieldset.fen-fieldset' ).first().attr( 'id' ) );
            // oElement.find( 'fieldset.fen-fieldset' ).incrementAttributes(
...
// console.log( oElement.find( 'fieldset.fen-fieldset' ).first().attr( 'id' ) );
     ...
     * Removes a repeatable field.
      This method is called when the user presses the - repeatable button.
     */
    $.fn.removefen_AdminPageFrameworkRepeatableField = function() {
        
        /* Need to remove the element: the field container */
        var nodeFieldContainer  = $( this ).closest( '.fen-field' );
        var nodeFieldsContainer = $( this ).closest( '.fen-fields' );
        var _sFieldsContainerID = nodeFieldsContainer.attr( 'id' );
        
        /* If the set minimum number of fields already exists, do not remove */
        var sMinNumberOfFields  = $.fn.afen_AdminPageFrameworkRepeatableFieldsOptions[ _sFieldsContainerID ][ 'min' ];
        if ( sMinNumberOfFields != 0 && nodeFieldsContainer.find( '.fen-field' ).length <= sMinNumberOfFields ) {
            var nodeLastRepeaterButtons = nodeFieldContainer.find( '.fen-repeatable-field-buttons' ).last();
            ...
            var _iFadeout = $.fn.afen_AdminPageFrameworkRepeatableFieldsOptions[ _sFieldsContainerID ][ 'fadeout' ]
                ? $.fn.afen_AdminPageFrameworkRepeatableFieldsOptions[ _sFieldsContainerID ][ 'fadeout' ]
                : 500;
            nodeMessage.delay( 2000 ).fadeOut( _iFadeout );
            return;     
        }     
