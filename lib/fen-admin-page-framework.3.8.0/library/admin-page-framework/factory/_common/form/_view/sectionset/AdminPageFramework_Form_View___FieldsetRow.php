...
class fen_AdminPageFramework_Form_View___FieldsetTableRow extends fen_AdminPageFramework_Form_View___Section_Base {
    ...
    public function get() {
        ...
        $_oFieldrowAttribute = new fen_AdminPageFramework_Form_View___Attribute_Fieldrow($aFieldset, array('id' => 'fieldrow-' . $aFieldset['tag_id'], 'valign' => 'top', 'class' => 'fen-fieldrow',));
        return $this->_getFieldByContainer($aFieldset, array('open_container' => "<tr " . $_oFieldrowAttribute->get() . ">", 'close_container' => "</tr>", 'open_title' => "<th>", 'close_title' => "</th>", 'open_main' => "<td " . $this->getAttributes(array('colspan' => $aFieldset['show_title_column'] ? 1 : 2, 'class' => $aFieldset['show_title_column'] ? null : 'fen-field-td-no-title',)) . ">", 'close_main' => "</td>",));
    }
    protected function _getFieldByContainer(array $aFieldset, array $aOpenCloseTags) {
        ...
        $_oFieldTitle = new fen_AdminPageFramework_Form_View___FieldTitle($aFieldset, '', $this->aSavedData, $this->aFieldErrors, $this->aFieldTypeDefinitions, $this->aCallbacks, $this->oMsg);
        $_aOutput = array();
    ...
    private function _getFieldTitle(array $aField) {
        $_oInputTagIDGenerator = new fen_AdminPageFramework_Form_View___Generate_FieldInputID($aField, 0);
        return "<label class='fen-field-title' for='" . $_oInputTagIDGenerator->get() . "'>" . "<a id='{$aField['field_id']}'></a>" . "<span title='" . esc_attr(strip_tags(is_array($aField['description']) ? implode('&#10;', $aField['description']) : $aField['description'])) . "'>" . $aField['title'] . $this->_getTitleColon($aField) . $this->_getToolTip($aField['tip'], $aField['field_id']) . "</span>" . "</label>";
    }
    private function _getToolTip($asTip, $sElementID) {
        $_oToolTip = new fen_AdminPageFramework_Form_View___ToolTip($asTip, $sElementID);
...
class fen_AdminPageFramework_Form_View___FieldsetRow extends fen_AdminPageFramework_Form_View___FieldsetTableRow {
    public $aFieldset = array();
    public $aSavedData = array();
    public $aFieldErrors = array();
    public $aFieldTypeDefinitions = array();
    ...
    public function get() {
        ...
        $_oFieldrowAttribute = new fen_AdminPageFramework_Form_View___Attribute_Fieldrow($aFieldset);
